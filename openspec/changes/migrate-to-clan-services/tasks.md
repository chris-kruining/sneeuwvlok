...
- [x] 1.2 Review existing `gateway`, `persistence`, `identity`, and `servarr` Clan services and document their current inputs, exports, drivers, and hardcoded repository-specific values.
...
- [x] 2.2 Refine the `gateway` interface so workload services can describe routes, endpoints, and reusable gateway functions without hardcoding gateway implementation details.
...
- [x] 3.2 Complete `gateway` with `driver = "caddy"` and explicit consumption of configured route/function inputs.
...
- [x] 5.1 Update `clan/instances.nix` so service instances pass persistence, gateway, identity, observability, and backup inputs explicitly.
...
- [x] 5.3 Ensure services do not implicitly discover provider services except through explicitly configured input scopes or values.
