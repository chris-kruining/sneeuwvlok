# clan-service-drivers Specification

## Purpose
TBD - Captures requirements for driver-backed Clan service wiring, discovery, and dispatch.
...
The shared Clan service library SHALL provide a helper that selects a service driver and role, invokes the selected role fragment functions with supplied arguments, wraps the selected role export set as a merge fragment list, and returns NixOS module fragment lists suitable for appending to lists passed to `mkMerge`.
...
- **WHEN** the selected role returns an export set and NixOS module fragments
- **THEN** the helper returns both as lists that can be appended to common service fragments
